---
title: Brachial Plexus
slug: brachial-plexus
phase: phase-1
subject: anatomy
systems:
  - upper-limb
summary: Learn the brachial plexus as an organized scaffold instead of a memorized tangle.
studyGoal: Understand roots, trunks, divisions, cords, and branches well enough to explain major lesion patterns.
audience: mbbs-foundation
maturity: reviewed
lastReviewed: 2026-04-05
highYield:
  - Build the structure in order before learning branches.
  - Anchor each cord around the axillary artery.
  - Clinical lesions make the anatomy easier to retain.
examFocus:
  - Viva sequencing
  - Diagram labeling
  - Erb palsy vs Klumpke palsy
clinicalBridge: Upper-limb weakness and sensory loss become much easier to localize once the plexus scaffold is stable.
sourceSlugs:
  - nmc-ug-curriculum
  - grays-anatomy-for-students
relatedTopics:
  - thoracic-wall-and-intercostal-spaces
---

## Big picture

The plexus is easier if students stop treating it as a giant list and start seeing it as a flowchart.

## How to study it

- memorize the main ladder first
- then map terminal branches
- then attach lesion patterns to specific levels

## What students usually mix up

- roots versus trunks
- cords versus branches
- upper trunk versus lower trunk lesions
