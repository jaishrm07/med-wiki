---
title: Glycolysis
slug: glycolysis
phase: phase-1
subject: biochemistry
systems:
  - metabolism
summary: The foundational glucose pathway that students should understand through purpose, regulation, and yield.
studyGoal: Learn glycolysis as an energy pathway with a clear input-output logic and a few regulatory checkpoints worth remembering.
audience: mbbs-foundation
maturity: reviewed
lastReviewed: 2026-04-05
highYield:
  - Focus on irreversible steps and regulation.
  - Know where ATP is spent and where ATP is gained.
  - Connect anaerobic glycolysis to clinical settings.
examFocus:
  - Rate-limiting steps
  - ATP accounting
  - Clinical importance
clinicalBridge: Tissue hypoxia, anaerobic exercise, hemolytic disorders, and pyruvate metabolism all connect back here.
sourceSlugs:
  - nmc-ug-curriculum
  - harpers-illustrated-biochemistry
diagramSlugs:
  - glycolysis-checkpoint-map
relatedTopics:
  - urea-cycle
---

## Best mental model

Think of glycolysis as the shortest path from glucose to quick usable energy.

## What to memorize selectively

- the irreversible checkpoints
- phosphofructokinase as the key control point
- net ATP and NADH production

## What to understand deeply

- why anaerobic glycolysis matters
- why regulation changes with cellular energy status
